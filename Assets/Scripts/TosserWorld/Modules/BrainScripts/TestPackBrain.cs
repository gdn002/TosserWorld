...
namespace TosserWorld.Modules.BrainScripts
{
    public class TestPackBrain : BrainScript
    {
        ...
        private void LeaderLoop()
        {
            ...
            GoTo(Destination);
            float angle = Vector2.SignedAngle(Vector2.up, Me.Movement.Movement);
                ...
                for (int i = 0; i < Pack.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (Vector2.Distance(Pack[i].Me.Position, pos) > 0.5f)
                            Pack[i].Me.Movement.SpeedLimit = Me.Movement.SpeedLimit * 1.5f;
                        else
                            Pack[i].Me.Movement.SpeedLimit = Me.Movement.SpeedLimit * 0.95f;
